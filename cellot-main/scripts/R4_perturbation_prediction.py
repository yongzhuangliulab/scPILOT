...
def predict_perturbation(
    ...
    for mode, experiment_name in [('ood', 'R4'), ('iid', 'R4hh')]:
        for query_no, query_key in enumerate(query_keys):
            print(f'======Predicting {drug_no + 1}: {drug_name} on {query_no + 1}: {query_key}======')
            path_ae = f'./results/{data_file}/holdout-{drug_name}_{query_key}/mode-{mode}/model-scgen'
            ...
            path_cellot = f'./results/{data_file}/holdout-{drug_name}_{query_key}/mode-{mode}/model-cellot'
            ...
            plt.savefig(f'./Figures/{data_file}_{drug_name}_prediction_{query_key}_{mode}.png', dpi = 200, bbox_inches = 'tight')
            ...
            _ = EGD_model(adata_query_eval).reg_mean_plot(
                ...
                path_to_save = f'./Figures/{data_file}_{drug_name}_reg_mean_{query_key}_{mode}.png',
                show = False,
                legend = False
            )
            plt.figure()
            _ = EGD_model(adata_query_eval).reg_var_plot(
                ...
                top_100_genes = diff_genes,
                labels = {'x': 'Prediction', 'y': 'Ground truth'},
                path_to_save = f'./Figures/{data_file}_{drug_name}_reg_var_{query_key}_{mode}.png',
                show = False,
                legend = False
            )
            ...
            plt.savefig(f'./Figures/{data_file}_{drug_name}_diff_genes[0]_violin_{query_key}_{mode}.png', dpi = 200, bbox_inches = 'tight')
            adata_query_eval.write_h5ad(f'./Result_anndata/{experiment_name}/{experiment_name}_CellOT_{data_file}_{query_key}_{drug_name}{file_type}')
