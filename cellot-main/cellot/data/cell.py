...
def load_cell_data(
    ...
    **kwargs
    ...
    if len(split_on) > 0:
        splits = {
            (key if isinstance(key, str) else ".".join(key)): data[data.obs_names.isin(index)]
...
def split_cell_data_toggle_ood(data, holdout, key, mode, random_state=0, **kwargs):
    ...
    print(value)
    if len(value) > 1:
        query_key_ctrl_obs_names = data.obs_names[data.obs[key] == value[1]]
        ...
        if mode == "ood":
            split.loc[ctrl_reserve_obs_names] = "ignore"
            split.loc[ctrl_holdout_obs_names] = "ood"

        elif mode == "iid":
            split.loc[ctrl_reserve_obs_names] = "train"
            split.loc[ctrl_holdout_obs_names] = "ood"

    query_key_stim_obs_names = data.obs_names[data.obs[key] == value[0]]
    ...
    if mode == "ood":
        split.loc[stim_reserve_obs_names] = "ignore"
        split.loc[stim_holdout_obs_names] = "ood"

    elif mode == "iid":
        split.loc[stim_reserve_obs_names] = "train"
        split.loc[stim_holdout_obs_names] = "ood"
