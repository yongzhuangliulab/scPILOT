...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Cell line to predict. Use all or omit to run all cell lines.",
)
parser.add_argument(
    "--seed",
    type=int,
...
def predict_one_query(
    ...
    target_gene_cell_type = f"{stim_key}_{query_key}"
    path_ae = (
        f"../cellot_results/{experiment_name}/"
        f"seed-{seed}/holdout-{target_gene_cell_type}/mode-ood/model-scgen"
    )
    path_cellot = (
        f"../cellot_results/{experiment_name}/"
