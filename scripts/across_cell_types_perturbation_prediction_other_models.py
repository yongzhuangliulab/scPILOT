...
parser = argparse.ArgumentParser(description='across_cell_types_perturbation_prediction_other_models')
parser.add_argument(
    '--model_name',
    type=str,
    default='biolord',
...
def make_prediction_obs_names_unique(adata_pred, model_name: str):
    adata_pred.obs_names = [f'{idx}_{model_name}_pred' for idx in adata_pred.obs_names.astype(str)]
    return adata_pred
...
def adjust_train_valid_counts(
    ...
    # Small deterministic adjustments around the nominal 90/10 split.
...
def fixed_shuffle_adata(adata, split_seed: int):
    """Return a copy with a deterministic order used by scvi's shuffle_set_split=False."""
...
def add_fixed_biolord_split_column(
    ...
    Held-out stimulated cells of the query cell type are assigned to 'test' and are
...
def evaluate_and_save(
    ...
    adata_query_pred = make_prediction_obs_names_unique(adata_query_pred.copy(), model_name=model_name)
    ...
    adata_query_eval_to_save = ad.concat([adata_query_ctrl, adata_query_stim, adata_query_pred])
    adata_query_eval_to_save.obs_names_make_unique()
    adata_query_eval_to_save.write_h5ad(
        f'../Result_anndata/{experiment_name}/{experiment_name}_{model_name}_{data_file}_{query_key}_seed{seed}{file_type}'
    )
...
def predict_perturbation(
    ...
    print(f'======Predicting {query_key} with {model_name} | seed={seed} | split_seed={split_seed}======')
    ...
    if model_name == 'biolord':
        ...
        # BioLORD does not expose shuffle_set_split in train(). Use split_key instead:
        # held-out stimulated query cells -> test; remaining cells -> deterministic train/valid split.
        set_seed(seed)
        ...
        set_seed(seed)
        idx_source = np.where(
            (adata_biolord.obs[cell_label_key] == query_key)
            & (adata_biolord.obs[cond_key] == ctrl_key)
        )[0]
        ...
        SCGEN.setup_anndata(train_scgen, batch_key=cond_key, labels_key=cell_label_key)
        ...
        raise ValueError(f'Unknown model_name: {model_name}. Expected one of: biolord, identity, scGen.')
