...
def plot_result(
    ...
    print('adata:')
    print(adata)
    sub_classes = sorted(adata.obs[sub_key].unique().tolist())
    for sub_no, sub_class in enumerate(sub_classes):
        query_adatas = []
        ...
        for model_no, model_name in enumerate(sorted(model_names)):
            ...
            print(model_name)
            adata_query_eval = adata_query_eval[adata_query_eval.obs[sub_key] == sub_class].copy()
            ...
            plt.savefig(f'../Figures/{experiment_name}/{model_name}_{data_file}_{query_key}_{sub_class}_prediction.jpg', dpi = 300, bbox_inches = 'tight')
            plt.close()
            if model_no == 0:
        ...
        for model_no, model_name in enumerate(model_names_in_order):
            ...
            plt.imshow(plt.imread(f'../Figures/{experiment_name}/{model_name.lower() if model_name in [
                'Biolord', 'Identity'
            ] else model_name}_{data_file}_{query_key}_{sub_class}_prediction.jpg'))
            plt.title(model_name)
        plt.savefig(f'../Figures/{experiment_name}/All_models_{data_file}_{query_key}_{sub_class}_prediction.jpg', dpi = 300, bbox_inches = 'tight')
        plt.close()
        end_time = time.time()
        print(f'======Prediction plotting for {sub_no + 1}: {query_key}_{sub_class} costs {end_time - start_time:.3f} secs.')
        start_time = time.time()
        ...
        for gene_no, gene_name in enumerate(important_genes):
            ...
            sns.despine(ax = plt.gca(), top = True, right = True)
            plt.savefig(f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_{sub_class}_{gene_name}_violin.jpg', dpi = 300, bbox_inches = 'tight')
            plt.close()
        grid = (1, 3)
        unit_len = 6
        loc = [(0, 0), (0, 1), (0, 2)]
        ...
        for gene_no, gene_name in enumerate(important_genes[: 3]):
            ...
            plt.axis('off')
            plt.imshow(plt.imread(f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_{sub_class}_{gene_name}_violin.jpg'))
        plt.savefig(f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_{sub_class}_important_genes_violin.jpg', dpi = 300, bbox_inches = 'tight')
        plt.close()
        end_time = time.time()
        print(f'======Violin plotting for {sub_no + 1}: {query_key}_{sub_class} costs {end_time - start_time:.3f} secs.')
    r2mean_dfs = []
    l2mean_dfs = []
    r2var_dfs = []
    l2var_dfs = []
    ...
    for model_no, model_name in enumerate(sorted(model_names)):
        ...
        start_time = time.time()
        for sub_no, sub_class in enumerate(sub_classes):
            print(f'======Plotting {model_no + 1}: {model_name}\'s result on {sub_no + 1}: {query_key}_{sub_class}======')
            adata_query_eval = ad.read_h5ad(f'../Result_anndata/{experiment_name}/{experiment_name}_{model_name}_{data_file}_{query_key}{file_type}')
            adata_query_eval = adata_query_eval[adata_query_eval.obs[sub_key] == sub_class].copy()
            adata_query_pred = adata_query_eval[adata_query_eval.obs[cond_key] == 'pred'].copy()
            adata_query_ctrl = adata_query_eval[adata_query_eval.obs[cond_key] == ctrl_key].copy()
            ...
            for gene_set in ['All genes', 'Top 100 DEGs']:
                ...
                print(f'{model_name} on {query_key}_{sub_class}: R2mean_{gene_set}_mean = {r2mean_values.mean()}, R2mean_{gene_set}_std = {r2mean_values.std()}')
                print(f'{model_name} on {query_key}_{sub_class}: L2mean_{gene_set}_mean = {l2mean_values.mean()}, L2mean_{gene_set}_std = {l2mean_values.std()}')
                print(f'{model_name} on {query_key}_{sub_class}: R2var_{gene_set}_mean = {r2var_values.mean()}, R2var_{gene_set}_std = {r2var_values.std()}')
                print(f'{model_name} on {query_key}_{sub_class}: L2var_{gene_set}_mean = {l2var_values.mean()}, L2var_{gene_set}_std = {l2var_values.std()}')
            ...
            print(f'{model_name} on {query_key}_{sub_class}: MMD_mean = {mmd_values.mean()}, MMD_std = {mmd_values.std()}')
        end_time = time.time()
        print(f'======Metrics computing for {model_no + 1}: {model_name} costs {end_time - start_time:.3f} secs.')
        gene_sets = ['All genes', 'Top 100 DEGs'] * len(sub_classes)
        sub_classes_fordf = []
        for sub_class in sub_classes:
            sub_classes_fordf.extend([sub_class] * 2)
        r2mean_df = pd.DataFrame({
            'R2mean means': r2mean_means,
            'R2mean stds': r2mean_stds,
            'R2mean values ndarrays': r2mean_values_list,
            'Gene set': gene_sets,
            'Sub class': sub_classes_fordf,
            'Model': [model_name] * 2 * len(sub_classes),
        })
        r2mean_df = r2mean_df.sort_values(by = 'Sub class')
        ...
        l2mean_df = l2mean_df.sort_values(by = 'Sub class')
        ...
        r2var_df = r2var_df.sort_values(by = 'Sub class')
        ...
        l2var_df = l2var_df.sort_values(by = 'Sub class')
        ...
        mmd_df = pd.DataFrame({
            ...
            'Model': [model_name] * len(sub_classes),
        })
        mmd_df = mmd_df.sort_values(by = 'Sub class')
        ...
        grouped_barplot(
            ...
            legend = True,
            filename = f'../Figures/{experiment_name}/{model_name}_on_{data_file}_{query_key}_barplot_R2mean.jpg',
            width = 2,
        ...
        grouped_barplot(
            ...
            legend = True,
            filename = f'../Figures/{experiment_name}/{model_name}_on_{data_file}_{query_key}_barplot_L2mean.jpg',
            width = 2,
        ...
        grouped_barplot(
            ...
            legend = True,
            filename = f'../Figures/{experiment_name}/{model_name}_on_{data_file}_{query_key}_barplot_R2var.jpg',
            width = 2,
        ...
        grouped_barplot(
            ...
            legend = True,
            filename = f'../Figures/{experiment_name}/{model_name}_on_{data_file}_{query_key}_barplot_L2var.jpg',
            width = 2,
        ...
        grouped_barplot(
            ...
            'MMD values ndarrays',
            legend = False,
            filename = f'../Figures/{experiment_name}/{model_name}_on_{data_file}_{query_key}_barplot_MMD.jpg',
            offset = 0,
            width = 1,
            ylabel = r'$\mathrm{MMD}$'
        )
    ...
    r2mean_final_df = r2mean_final_df.sort_values(by = ['Sub class', 'Model'])
    r2mean_final_df.to_pickle(f'../DataFrames/{experiment_name}/{query_key}_r2mean_final.pkl')
    l2mean_final_df = l2mean_final_df.sort_values(by = ['Sub class', 'Model'])
    l2mean_final_df.to_pickle(f'../DataFrames/{experiment_name}/{query_key}_l2mean_final.pkl')
    r2var_final_df = r2var_final_df.sort_values(by = ['Sub class', 'Model'])
    r2var_final_df.to_pickle(f'../DataFrames/{experiment_name}/{query_key}_r2var_final.pkl')
    l2var_final_df = l2var_final_df.sort_values(by = ['Sub class', 'Model'])
    l2var_final_df.to_pickle(f'../DataFrames/{experiment_name}/{query_key}_l2var_final.pkl')
    mmd_final_df = mmd_final_df.sort_values(by = ['Sub class', 'Model'])
    mmd_final_df.to_pickle(f'../DataFrames/{experiment_name}/{query_key}_mmd_final.pkl')
    ...
    grouped_barplot(
        ...
        'R2mean values ndarrays',
        legend = True,
        filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_barplot_R2mean.jpg',
        offset = 1.875,
        width = 6,
        ylabel = r'$\mathrm{R^2_{\mathrm{\mathsf{mean}}}}$'
    )
    grouped_barplot(
        ...
        'R2mean values ndarrays',
        legend = True,
        filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_barplot_R2mean_T100.jpg',
        offset = 1.875,
        width = 6,
    ...
    grouped_barplot(
        ...
        'L2mean values ndarrays',
        legend = True,
        filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_barplot_L2mean.jpg',
        offset = 1.875,
        width = 6,
        ylabel = r'$\mathrm{L^2_{\mathrm{\mathsf{mean}}}}$'
    )
    grouped_barplot(
        ...
        'L2mean values ndarrays',
        legend = True,
        filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_barplot_L2mean_T100.jpg',
        offset = 1.875,
        width = 6,
    ...
    grouped_barplot(
        ...
        'R2var values ndarrays',
        legend = True,
        filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_barplot_R2var.jpg',
        offset = 1.875,
        width = 6,
        ylabel = r'$\mathrm{R^2_{\mathrm{\mathsf{var}}}}$'
    )
    grouped_barplot(
        ...
        'R2var values ndarrays',
        legend = True,
        filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_barplot_R2var_T100.jpg',
        offset = 1.875,
        width = 6,
    ...
    grouped_barplot(
        ...
        'L2var values ndarrays',
        legend = True,
        filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_barplot_L2var.jpg',
        offset = 1.875,
        width = 6,
        ylabel = r'$\mathrm{L^2_{\mathrm{\mathsf{var}}}}$'
    )
    grouped_barplot(
        ...
        'L2var values ndarrays',
        legend = True,
        filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_barplot_L2var_T100.jpg',
        offset = 1.875,
        width = 6,
    ...
    grouped_barplot(
        ...
        'MMD values ndarrays',
        legend = True,
        filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_barplot_MMD.jpg',
        offset = 1.875,
        width = 6,
        ylabel = r'$\mathrm{MMD}$'
    )
if __name__ == '__main__':
    ...
    plot_result(query_key = query_key)
    print('Done')