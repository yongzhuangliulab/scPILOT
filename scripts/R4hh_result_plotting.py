    ...
    for drug_no, drug_name in enumerate(drug_names):
        ...
        for query_no, query_key in enumerate(query_keys):
            ...
            for model_no, model_name in enumerate(sorted(model_names)):
                adata_query_eval = ad.read_h5ad(f'../Result_anndata/{experiment_name}/{experiment_name}_{model_name}_{data_file}_{query_key}_{drug_name}{file_type}')
                adata_query_ctrl = adata_query_eval[adata_query_eval.obs[cond_key] == ctrl_key].copy()
                adata_query_stim = adata_query_eval[adata_query_eval.obs[cond_key] == drug_name].copy()
                adata_query_pred = adata_query_eval[adata_query_eval.obs[cond_key] == 'pred'].copy()
                adata_query_ctst = ad.concat([adata_query_ctrl, adata_query_stim])
                adata_query_stpr = ad.concat([adata_query_stim.copy(), adata_query_pred.copy()])
                adata_query_ctst.obs = adata_query_ctst.obs.replace(drug_name, drug_name.capitalize()).replace('control', 'Control')
                adata_query_stpr.obs = adata_query_stpr.obs.replace(drug_name, drug_name.capitalize()).replace('pred', 'Pred')
                adata_query_ctst.obs.rename(columns = {cond_key: cond_key.capitalize()}, inplace = True)
                adata_query_stpr.obs.rename(columns = {cond_key: cond_key.capitalize()}, inplace = True)
                sc.pp.pca(adata_query_ctst)
                sc.pp.pca(adata_query_stpr)
                sc.pp.neighbors(adata_query_ctst)
                sc.pp.neighbors(adata_query_stpr)
                sc.tl.umap(adata_query_ctst)
                sc.tl.umap(adata_query_stpr)
                sc.pl.umap(adata_query_ctst, color = cond_key.capitalize(), palette = {'Control': '#4C72B0', drug_name.capitalize(): '#DD8452'}, title = '', frameon = False, show = False)
                plt.savefig(f'../Figures/{experiment_name}/{data_file}_{query_key}_{drug_name}_UMAP.png', dpi = 200, bbox_inches = 'tight')
                if model_no < len(model_names) - 1:
                    sc.pl.umap(adata_query_stpr, color = cond_key.capitalize(), palette = {'Pred': '#4C72B0', drug_name.capitalize(): '#DD8452'}, legend_loc = None, title = '', frameon = False, show = False)
                else:
                    sc.pl.umap(adata_query_stpr, color = cond_key.capitalize(), palette = {'Pred': '#4C72B0', drug_name.capitalize(): '#DD8452'}, title = '', frameon = False, show = False)
                plt.savefig(f'../Figures/{experiment_name}/{model_name}_{data_file}_{query_key}_{drug_name}_prediction.png', dpi = 200, bbox_inches = 'tight')
                plt.close()
                if model_no == 0:
            ...
            for model_no, model_name in enumerate(model_names_in_order):
                ...
                plt.imshow(plt.imread(f'../Figures/{experiment_name}/{model_name.lower() if model_name in [
                    'Biolord', 'Identity'
                ] else model_name}_{data_file}_{query_key}_{drug_name}_prediction.png'))
                plt.title(model_name)
            plt.savefig(f'../Figures/{experiment_name}/All_models_{data_file}_{query_key}_{drug_name}_prediction.png', dpi = 400, bbox_inches = 'tight')
            plt.close()
            ...
            ).replace(drug_name, drug_name.capitalize()).replace('biolord', 'Biolord')
            groupby_order = [ctrl_key.capitalize(), drug_name.capitalize(), 'Biolord', 'CellOT', 'VAEGAN', 'scGen', 'scPILOT']
            ...
            important_genes = adata_query.uns['rank_genes_groups']['names'][drug_name]
            for gene_no, gene_name in enumerate(important_genes):
                ...
                plt.savefig(f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_{drug_name}_{gene_name}_violin.png', dpi = 200, bbox_inches = 'tight')
            grid = (1, 3)
            unit_len = 6
            loc = [(0, 0), (0, 1), (0, 2)]
            colspan = 1
            ...
            for gene_no, gene_name in enumerate(important_genes[: 3]):
                ...
                plt.axis('off')
                plt.imshow(plt.imread(f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_{drug_name}_{gene_name}_violin.png'))
            plt.savefig(f'../Figures/{experiment_name}/All_models_on_{data_file}_{query_key}_{drug_name}_important_genes_violin.png', dpi = 400, bbox_inches = 'tight')
            plt.close()
        for model_no, model_name in enumerate(sorted(model_names)):
            r2mean_means = []
            r2mean_stds = []
            ...
            mmd_stds = []
            for query_no, query_key in enumerate(query_keys):
                print(f'======Plotting {model_no + 1}: {model_name}\'s result on {query_no + 1}: {query_key}, {drug_no + 1}: {drug_name}======')
                adata_query_eval = ad.read_h5ad(f'../Result_anndata/{experiment_name}/{experiment_name}_{model_name}_{data_file}_{query_key}_{drug_name}{file_type}')
                adata_query_pred = adata_query_eval[adata_query_eval.obs[cond_key] == 'pred'].copy()
                adata_query_ctrl = adata_query_eval[adata_query_eval.obs[cond_key] == ctrl_key].copy()
                adata_query_stim = adata_query_eval[adata_query_eval.obs[cond_key] == drug_name].copy()
                adata_query = ad.concat([adata_query_ctrl.copy(), adata_query_stim.copy()])
                sc.tl.rank_genes_groups(adata_query, groupby = cond_key, method = 'wilcoxon', n_genes = 100)
                ...
                for gene_set in ['All genes', 'Top 100 DEGs']:
                    ...
                    print(f'{model_name} on {query_key}_{drug_name}: R2mean_{gene_set}_mean = {r2mean_values.mean()}, R2mean_{gene_set}_std = {r2mean_values.std()}')
                    print(f'{model_name} on {query_key}_{drug_name}: L2mean_{gene_set}_mean = {l2mean_values.mean()}, L2mean_{gene_set}_std = {l2mean_values.std()}')
                    print(f'{model_name} on {query_key}_{drug_name}: R2var_{gene_set}_mean = {r2var_values.mean()}, R2var_{gene_set}_std = {r2var_values.std()}')
                    print(f'{model_name} on {query_key}_{drug_name}: L2var_{gene_set}_mean = {l2var_values.mean()}, L2var_{gene_set}_std = {l2var_values.std()}')
                ...
                print(f'{model_name} on {query_key}_{drug_name}: MMD_mean = {mmd_values.mean()}, MMD_std = {mmd_values.std()}')
            ...
            print(f'{model_name}_{drug_name} R2mean_df:')
            print(r2mean_df)
            ...
            print(f'{model_name}_{drug_name} L2mean_df:')
            print(l2mean_df)
            ...
            print(f'{model_name}_{drug_name} R2var_df:')
            print(r2var_df)
            ...
            print(f'{model_name}_{drug_name} L2var_df:')
            print(l2var_df)
            mmd_df = pd.DataFrame({
                'MMD means': mmd_means,
                'MMD stds': mmd_stds,
            ...
            print(f'{model_name}_{drug_name} MMD_df:')
            print(mmd_df)
            grouped_barplot(
                r2mean_df,
                'Cell label',
                ...
                'R2mean stds',
                legend = True,
                filename = f'../Figures/{experiment_name}/{model_name}_on_{data_file}_{drug_name}_barplot_R2mean.png',
                width = 2,
            ...
            grouped_barplot(
                ...
                'L2mean stds',
                legend = True,
                filename = f'../Figures/{experiment_name}/{model_name}_on_{data_file}_{drug_name}_barplot_L2mean.png',
                width = 2,
            ...
            grouped_barplot(
                ...
                'R2var stds',
                legend = True,
                filename = f'../Figures/{experiment_name}/{model_name}_on_{data_file}_{drug_name}_barplot_R2var.png',
                width = 2,
            ...
            grouped_barplot(
                ...
                'L2var stds',
                legend = True,
                filename = f'../Figures/{experiment_name}/{model_name}_on_{data_file}_{drug_name}_barplot_L2var.png',
                width = 2,
            ...
            grouped_barplot(
                ...
                'MMD stds',
                legend = False,
                filename = f'../Figures/{experiment_name}/{model_name}_on_{data_file}_{drug_name}_barplot_MMD.png',
                offset = 0,
                width = 1,
                ylabel = r'$\mathrm{MMD}$'
            )
        ...
        grouped_barplot(
            ...
            put_label = False,
            legend = True,
            filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{drug_name}_barplot_R2mean.png',
            offset = 1.875,
            width = 6,
            ylabel = r'$\mathrm{R^2_{\mathrm{\mathsf{mean}}}}$'
        )
        grouped_barplot(
            ...
            put_label = False,
            legend = True,
            filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{drug_name}_barplot_L2mean.png',
            offset = 1.875,
            width = 6,
            ylabel = r'$\mathrm{L^2_{\mathrm{\mathsf{mean}}}}$'
        )
        grouped_barplot(
            ...
            put_label = False,
            legend = True,
            filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{drug_name}_barplot_R2var.png',
            offset = 1.875,
            width = 6,
            ylabel = r'$\mathrm{R^2_{\mathrm{\mathsf{var}}}}$'
        )
        grouped_barplot(
            ...
            put_label = False,
            legend = True,
            filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{drug_name}_barplot_L2var.png',
            offset = 1.875,
            width = 6,
            ylabel = r'$\mathrm{L^2_{\mathrm{\mathsf{var}}}}$'
        )
        grouped_barplot(
            ...
            put_label = False,
            legend = True,
            filename = f'../Figures/{experiment_name}/All_models_on_{data_file}_{drug_name}_barplot_MMD.png',
            offset = 1.875,
            width = 6,
            ylabel = r'$\mathrm{MMD}$'
        )
