...
for seed in 1327 1337 1347; do
    for model_name in biolord identity scGen; do
        log_dir="../Logs/across_cell_types/${model_name}/seed-${seed}"
        mkdir -p "${log_dir}"
        for query_key in $(cat ../Data/across_cell_types/cell_labels.txt); do
            echo \
                nohup python across_cell_types_perturbation_prediction_other_models.py \
                --model_name ${model_name} \
                --query_key ${query_key} \
                --seed ${seed} \
                --split_seed 0 \
                \> "${log_dir}/across_cell_types_${model_name}_${query_key}_seed${seed}.log" \
                2\>\&1 \
                \&;
        done;
    done;
