    ...
    for model_no, model_name in enumerate(sorted(model_names)):
        ...
        for query_no, query_key in enumerate(query_keys):
            ...
            for gene_set in ['All genes', 'Top 100 DEGs']:
                ...
                for j in range(100):
                    ...
                    if gene_set == 'All genes':
                        x = np.average(adata_query_pred.X[adata_query_pred_idx], axis = 0)
                        y = np.average(adata_query_stim.X[adata_query_stim_idx], axis = 0)
                    else:
                        x = np.average(adata_query_pred[: , diff_genes.tolist()].X[adata_query_pred_idx], axis = 0)
                        y = np.average(adata_query_stim[: , diff_genes.tolist()].X[adata_query_stim_idx], axis = 0)
                    _, _, r_value, _, _ = stats.linregress(x, y)
                    r2mean_values[0, j] = r_value ** 2
                    l2mean_values[0, j] = sqrt(((x - y) ** 2).sum())
                    if gene_set == 'All genes':
                        x = np.var(adata_query_pred.X[adata_query_pred_idx], axis = 0)
                        y = np.var(adata_query_stim.X[adata_query_stim_idx], axis = 0)
                    else:
                        x = np.var(adata_query_pred[: , diff_genes.tolist()].X[adata_query_pred_idx], axis = 0)
                        y = np.var(adata_query_stim[: , diff_genes.tolist()].X[adata_query_stim_idx], axis = 0)
                    _, _, r_value, _, _ = stats.linregress(x, y)
            ...
            for j in range(10):
                adata_query_pred_idx = np.random.choice(range(adata_query_pred.shape[0]), int(0.8 * adata_query_pred.shape[0]))
                adata_query_stim_idx = np.random.choice(range(adata_query_stim.shape[0]), int(0.8 * adata_query_stim.shape[0]))
                x = adata_query_pred[: , diff_genes.tolist()[: 50]].X[adata_query_pred_idx]
                y = adata_query_stim[: , diff_genes.tolist()[: 50]].X[adata_query_stim_idx]
