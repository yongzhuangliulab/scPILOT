...
parser = argparse.ArgumentParser(
    description="Across-patient query-reference mismatch stress test."
)
parser.add_argument("--query_key", type=int, required=True)
parser.add_argument(
    "--seed",
    type=int,
    required=True,
...
def main() -> None:
    ...
    stress_name = "across_patients_reference_mismatch"
    ...
    adata_query_stim = adata[
        obs_value_mask(adata, patient_key, args.query_key)
        & obs_value_mask(adata, cond_key, stim_key)
    ].copy()
    adata_query = adata[
        obs_value_mask(adata, patient_key, args.query_key)
