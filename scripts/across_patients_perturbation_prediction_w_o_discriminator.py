...
parser = argparse.ArgumentParser(description='across_patients_perturbation_prediction_w_o_discriminator')
parser.add_argument(
    '--query_key',
    type=int,
    default=101,
...
class EpochProgressPrinter(Callback):
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        for key in [
            "VAE_loss_train", "elbo_train", "rl_train", "kld_train", "dl_train",
...
def predict_perturbation(
    ...
    # Outer hold-out setting is unchanged:
    # only the stimulated cells of the query patient are excluded.
    train = adata[
        ~(
    ...
    # To reuse an already trained ablation checkpoint, comment out the
    # training/save block above and enable the following line.
    ...
    r2mean_all, r2mean_top50 = EGD_model.reg_mean_plot(
        ...
        path_to_save=(
            f'../Figures/{experiment_name}/'
            f'{eval_model_name}_{data_file}_reg_mean_{query_key}_seed{seed}.jpg'
        ),
    ...
    adata_query_eval_to_save.write_h5ad(
        f'../Result_anndata/{experiment_name}/'
        f'{experiment_name}_{eval_model_name}_{data_file}_'
        f'{query_key}_seed{seed}{file_type}'
    )
