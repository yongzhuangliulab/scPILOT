        ...
        for seed in seeds:
            ...
            for model_name in model_order:
                ...
                if not truth_added:
                    ...
                    adata_ctrl_plot.obs[plot_group_key] = condition_display_name(ctrl_key)
                    adata_stim_plot.obs[plot_group_key] = condition_display_name(stim_key)
...
def paired_test_scpilot_vs_baseline(query_summary, metric, baseline, test_name):
    ...
    Each paired sample is one held-out cell line.
    The value for each held-out cell line is the average across three seeds.
    """
...
def summarize_worst_case_performance(
    ...
    """
    Summarize worst-case performance across held-out cell lines.
...
def plot_worst_case_performance_combined(
    ...
    fig.suptitle(
        'Mean and worst-case performance across held-out cell lines',
        fontsize=16,
        x=0.4,
        y=1.03,
    )
...
def plot_splitwise_radar_chart(
    ...
    for i, model_name in enumerate(model_order):
        ...
        if sd_col in dfg_mean.columns:
            ...
            # Optional error bars showing mean ± SD at each held-out cell line.
            if show_error_bars:
...
def plot_splitwise_line_chart(
    ...
    ax.set_xlabel('Held-out cell line', fontsize=12)
...
def plot_aggregate_boxplot_combined(
    ...
    Each box summarizes split-level mean values across held-out cell lines.
    ...
    fig.suptitle(
        f'Aggregate performance across held-out cell lines ({test_title})',
        fontsize=15,
        y=1.03,
    )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Across-cell-line result plotting and summary.')
