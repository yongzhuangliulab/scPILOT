...
def plot_shared_umaps_from_h5ad(
    ...
    output_name='across_species_ablation',
...
def summarize_recovery_gene_level(
    ...
    # Paired two-sided Wilcoxon on four query-level seed averages.
    wilcoxon_records = []
    variants = [
        model_name
        for model_name in model_names
...
def paired_test_reference_vs_variant(
    ...
    Each paired observation is one held-out species.
    Values are first averaged across model seeds within each species.
    """
...
def summarize_worst_case_performance(
    ...
    """
    Summarize worst-case performance across held-out species.
...
def plot_worst_case_performance_combined(
    ...
    fig.suptitle(
        'Mean and worst-case performance across held-out species',
        fontsize=16,
        x=0.4,
        y=1.03,
    )
...
def plot_aggregate_boxplot_combined(
    ...
    Each box summarizes split-level mean values across held-out species.
    ...
    fig.suptitle(
        f'Aggregate performance across held-out species ({test_title})',
        fontsize=15,
        y=1.03,
    )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Across-species ablation result plotting and summary.'
    )
    ...
    parser.add_argument(
        '--output_name',
        type=str,
        default='across_species_ablation',
    )
