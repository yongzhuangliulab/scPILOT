...
for query_key in $(cat ../Data/across_patients/sample_ids.txt); do
    log_dir="../Logs/across_patients/cell_type_ablation/query-${query_key}"
    mkdir -p "${log_dir}"
    echo \
        nohup python across_patients_cell_type_ablation.py \
        --query_key "${query_key}" \
        --seed all \
        --output_name across_patients_ablation \
        \> "${log_dir}/across_patients_cell_type_ablation_${query_key}.log" \
        2\>\&1 \
        \&
done
