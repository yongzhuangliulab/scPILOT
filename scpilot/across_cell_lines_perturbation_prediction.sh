...
for seed in 1327 1337 1347; do
    log_dir="../Logs/across_cell_lines/scPILOT/seed-${seed}"
    mkdir -p "${log_dir}"

    for query_key in $(cat ../Data/across_cell_lines/cell_types.txt); do
        echo \
            nohup python across_cell_lines_perturbation_prediction.py \
            --query_key ${query_key} \
            --seed ${seed} \
            --split_seed 0 \
            \> "${log_dir}/across_cell_lines_scPILOT_${query_key}_seed${seed}.log" \
            2\>\&1 \
            \&;
    done;
done;
