...
for seed in 1327 1337 1347; do
    log_dir="../Logs/across_patients/scPILOT/seed-${seed}"
    mkdir -p "${log_dir}"

    for query_key in $(cat ../Data/across_patients/sample_ids.txt); do
        echo \
            nohup python across_patients_perturbation_prediction.py \
            --query_key ${query_key} \
            --seed ${seed} \
            --split_seed 0 \
            \> "${log_dir}/across_patients_scPILOT_${query_key}_seed${seed}.log" \
            2\>\&1 \
            \&;
    done;
done;