...
parser = argparse.ArgumentParser(description="CellOT across-patient perturbation prediction")
parser.add_argument(
    "--query_key",
    type=str,
    default=None,
    help="Patient/sample ID to predict. Use all or omit to run all patients.",
)
parser.add_argument(
    "--seed",
    type=int,
...
def predict_one_query(
    ...
    condition_sample_id = f"{stim_key}_{query_key}"

    path_ae = (
        f"../cellot_results/{experiment_name}/"
        f"seed-{seed}/holdout-{condition_sample_id}/mode-ood/model-scgen"
    )
    path_cellot = (
        f"../cellot_results/{experiment_name}/"
    ...
    adata_query_ctrl = adata[
        query_mask & (adata.obs[cond_key] == ctrl_key)
    ].copy()
    adata_query_stim = adata[
        query_mask & (adata.obs[cond_key] == stim_key)
    ].copy()
    ...
    adata_query_pred.obs_names = [
        f"{idx}_pred"
        for idx in adata_query_ctrl.obs_names.astype(str)
    ]
