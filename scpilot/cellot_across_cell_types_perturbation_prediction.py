...
parser = argparse.ArgumentParser(description="CellOT across-cell-type perturbation prediction")
parser.add_argument("--query_key", type=str, default=None, help="Cell type to predict. Use all or omit to run all cell types.")
parser.add_argument("--seed", type=int, default=1327, help="Training/inference seed. Recommended: 1327, 1337, 1347.")
parser.add_argument("--split_seed", type=int, default=0, help="Recorded split seed for consistency with other methods.")
...
def predict_one_query(
    ...
    set_seed(seed)
    print(f"======Predicting {query_key} | seed={seed} | split_seed={split_seed}======", flush=True)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}", flush=True)

    # Determined by condition_cell_types.txt: stimulated_B, stimulated_CD14+Mono, ...
    condition_cell_type = f"{stim_key}_{query_key}"
    path_ae = f"../cellot_results/{experiment_name}/seed-{seed}/holdout-{condition_cell_type}/mode-ood/model-scgen"
    path_cellot = f"../cellot_results/{experiment_name}/seed-{seed}/holdout-{query_key}/mode-ood/model-cellot"
    ...
    inputs = torch.as_tensor(to_dense_array(adata_query_ctrl.X), dtype=torch.float32, device=device)
    with torch.no_grad():
    ...
    inputs = torch.as_tensor(to_dense_array(latent_query_ctrl.X), dtype=torch.float32, device=device)
    ...
    latent_inputs = torch.as_tensor(transported, dtype=torch.float32, device=device)
    with torch.no_grad():
    ...
    adata_query_pred.obs_names = [f"{idx}_pred" for idx in adata_query_ctrl.obs_names]
    ...
    r2mean_all, r2mean_top50 = EGD_model.reg_mean_plot(
        ...
        path_to_save=f"../Figures/{experiment_name}/CellOT_{data_file}_reg_mean_{query_key}_seed{seed}.jpg",
    ...
    adata_query_eval_to_save.write_h5ad(
        f"../Result_anndata/{experiment_name}/{experiment_name}_CellOT_{data_file}_{query_key}_seed{seed}{file_type}"
    )
    ...
    metrics_df.to_csv(
        f"../DataFrames/{experiment_name}/{experiment_name}_{data_file}_{query_key}_seed{seed}_CellOT_metrics.csv",
        index=False,
    )
